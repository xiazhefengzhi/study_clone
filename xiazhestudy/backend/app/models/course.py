...
class Course(Base):
    """
    Course/Lesson model - 课程/动画模型

    状态说明：
    - pending: 排队中（刚创建，等待生成）
    - processing: 生成中（正在AI生成动画）
    - completed: 已完成（生成成功）
    - failed: 失败（生成失败）
    """
    ...
    status: Mapped[str] = mapped_column(
        ...
        default="pending",
        nullable=False,
        comment="课程状态：pending/processing/completed/failed"
    )
    ...
    # Likes Relationship（点赞关系）
    likes: Mapped[List["CourseLike"]] = relationship(
        "CourseLike",
